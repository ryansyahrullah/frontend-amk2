<template>
  <div class="space-y-6 sm:space-y-8">
    <section class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary to-indigo-600 p-5 text-white shadow-glow sm:p-8">
      ...
      <div class="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between lg:gap-10">
        <div class="max-w-2xl">
          <p class="text-xs uppercase tracking-[0.35em] text-white/70 sm:text-sm">Ringkasan AMK</p>
          <h1 class="mt-2 text-xl font-semibold leading-tight sm:mt-3 sm:text-3xl lg:text-[2.5rem]">Dashboard SDM modern untuk memantau kinerja tim Anda.</h1>
          <p class="mt-3 text-xs leading-relaxed text-white/75 sm:text-sm">
    ...
    <div class="grid gap-6 xl:grid-cols-3">
      <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6 xl:col-span-2">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100 sm:text-lg">Performa Program Pengembangan</h2>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 sm:text-sm">Perbandingan aktivitas pelatihan dan onboarding per bulan.</p>
          </div>
          <div class="flex gap-2 text-[0.65rem] font-medium text-slate-500 dark:text-slate-400 sm:text-xs">
        ...
        <div class="mt-5 grid gap-5 sm:mt-6 sm:gap-6 lg:grid-cols-2">
          <div>
            <div class="h-52 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60 sm:h-56 sm:p-5">
              <div class="flex h-full items-end gap-3 sm:gap-4">
                <div
                  v-for="item in pelatihanSeries"
                  :key="item.label"
                  class="flex flex-1 flex-col items-center gap-1.5 text-[0.65rem] font-medium text-slate-500 dark:text-slate-400 sm:text-xs"
                >
          ...
          <div>
            <div class="h-52 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60 sm:h-56 sm:p-5">
              ...
              <div class="mt-3 flex items-center justify-between text-[0.65rem] text-slate-500 dark:text-slate-400 sm:mt-4 sm:text-xs">
      ...
      <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100 sm:text-lg">Komposisi Status Pegawai</h2>
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400 sm:text-sm">Distribusi status kepegawaian berdasarkan data terbaru.</p>
        <ul class="mt-4 space-y-3 sm:mt-5 sm:space-y-4">
          <li v-for="status in statusSummary" :key="status.label" class="space-y-1.5 sm:space-y-2">
            <div class="flex items-center justify-between text-xs font-medium text-slate-700 dark:text-slate-200 sm:text-sm">
            ...
            <p class="text-[0.65rem] text-slate-500 dark:text-slate-400 sm:text-xs">{{ status.percent }}% dari total pegawai</p>
          </li>
        </ul>
      </div>
    </div>
    ...
    <div class="grid gap-5 sm:gap-6 lg:grid-cols-3">
      <section class="lg:col-span-2">
        <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
          <div class="flex flex-wrap items-center justify-between gap-3 sm:gap-4">
            <div>
              <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100 sm:text-lg">Pegawai Akan Habis Kontrak</h2>
              <p class="text-xs text-slate-500 dark:text-slate-400 sm:text-sm">Pantau jadwal kontrak yang segera berakhir.</p>
            </div>
            <RouterLink to="/pegawai?filter=habis-kontrak" class="text-xs font-semibold text-primary hover:text-primary-dark sm:text-sm">
              Lihat Semua
            </RouterLink>
          </div>
      ...
      <section>
        <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
          <h2 class="text-base font-semibold text-slate-900 dark:text-slate-100 sm:text-lg">Riwayat Aktivitas</h2>
          <p class="text-xs text-slate-500 dark:text-slate-400 sm:text-sm">10 aktivitas terbaru dari seluruh pengguna.</p>
          <ul class="mt-3 space-y-3 sm:mt-4 sm:space-y-4">
            <li v-if="loadingAktivitas" v-for="n in 5" :key="`skeleton-${n}`" class="animate-pulse space-y-2.5 sm:space-y-3">
            ...
            <li
              ...
              class="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-slate-300 p-5 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400 sm:p-6 sm:text-sm"
            >
              <span class="text-3xl">📭</span>
              Belum ada aktivitas terbaru.
            </li>
            <li v-else v-for="item in aktivitas" :key="item.id" class="rounded-2xl border border-slate-200 p-4 text-xs dark:border-slate-800 sm:text-sm">
              ...
              <p class="mt-2 text-[0.65rem] text-slate-400 dark:text-slate-500 sm:text-xs">{{ formatDateTime(item.waktu) }}</p>
            </li>
          </ul>
        </div>
      </section>
